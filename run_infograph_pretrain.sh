...
#SBATCH --mem=48G
#SBATCH --time=3-00:00
#SBATCH -o /mnt/qb/work/akata/aoq877/repsssl/LOGS/infog-pretrain-j-%j.out
#SBATCH --array=14
...
python main_pretrain.py \
        ...
        --config-name pretrain.yaml \
            ...
            ++seed=123 \
            ++data.dataset="infograph" \
            ++data.num_classes=345 \
            ++data.num_workers=10 \
            ++optimizer.batch_size='auto' \
            ++optimizer.lr=0.01 \
            ++wandb.project='Infograph_Pretrain' \
            ++model_id="${models[$SLURM_ARRAY_TASK_ID]}" \
            ++search_id=None \
            ++max_epochs=50 \
            ++checkpoint.enabled=False \
            ++freeze=False \
            ++mode='Infograph_Finetune_Test'
