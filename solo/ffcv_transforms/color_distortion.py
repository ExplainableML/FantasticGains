...
class RandomColorDistortion(Operation):
    """Add ColorDistortion with probability p.
    Operates on raw arrays (not tensors).
    Parameters
    ----------
    p : float, The probability with which to apply ColorDistortion.
    strength: float, the strength with which distortion is applied.
    """

    def __init__(self, strength=0.5, p=0.5):
        super().__init__()
        if isinstance(strength, list):
            brightness = 0.8 * strength[0]
            contrast = 0.8 * strength[1]
            saturation = 0.8 * strength[2]
            hue = 0.2 * strength[3]
        else:
    ...
    def generate_code(self) -> Callable:
        ...
        def color_jitter(images, dst):
            should_jitter = rand(images.shape[0]) < p
