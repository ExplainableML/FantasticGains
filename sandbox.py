...
if __name__ == "__main__":
    with open('plots/prediction_flips/124_prediction_flips.json') as f:
        p124 = json.load(f)
    with open('plots/prediction_flips/165_prediction_flips.json') as f:
        p165 = json.load(f)
    with open('plots/prediction_flips/189_prediction_flips.json') as f:
        p189 = json.load(f)
    p_values = [2, 3, 5, 10, 20]
    data = []
    for r, run in enumerate([p124, p189, p165]):
        k, avg_sim, max_sim, share = get_topk_class_sim(np.array(run['results']['pos_class_flips']), p=p_values)
        for p, p_share in enumerate(p_values):
            data.append([r, run['config']['ts_diff'], run['results']['mean_pos_class_flips'], run['results']['ent_pos_class_flips'], k[p], avg_sim[p], max_sim[p], share[p]])
    data = pd.DataFrame(data=data, columns=['run', 'ts_diff', 'pos_flips', 'entropy', 'k', 'avg_sim', 'max_sim', 'share'])
    test